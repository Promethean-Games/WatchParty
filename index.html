  ...
  <div id="app">
    <header class="app-header">
      ...
      <div class="header-right">
        ...
        <button id="googleSignInBtn" class="btn btn-ghost small hidden">Sign in with Google</button>
    ...
    <main class="app-main">
      ...
      <section id="screen-setup" class="screen active">
        <!-- Host/Party Tab Navigation -->
        <div class="setup-tabs">
          <button id="hostTabBtn" class="setup-tab active">Host</button>
          <button id="partyTabBtn" class="setup-tab">Party</button>
        </div>

        <!-- Host Panel -->
        <div id="hostPanel" class="setup-panel active">
          <div class="card">
            <h2>Host Setup</h2>
            ...
            <label class="field">
              <div class="field-label-row">
                ...
                <span class="field-hint">
                  Share this code so friends can join from their phones.
                </span>
              </div>
            ...
            <div class="field">
              <div class="field-label-row">
                ...
                <span class="subtle">Add local players on this device.</span>
              </div>
              <div class="player-input-row">
                <input id="playerNameInput" type="text" maxlength="18" placeholder="Add player name" />
                <button id="playerAvatarBtn" class="btn btn-ghost avatar-picker-btn" type="button">
                  <span id="playerAvatarPreview">🎮</span>
                </button>
            ...
            <div class="field team-mode-field">
              <div class="field-label-row">
                <span class="field-label">Team Play</span>
                <label class="toggle-switch">
                  <input type="checkbox" id="teamModeToggle" />
                  <span class="toggle-slider"></span>
                </label>
              </div>
              <span class="subtle" id="teamModeHint">Off: individual scores. On: Blue vs Red team scores.</span>
            </div>

            <div id="teamRosterSection" class="team-roster-section hidden">
              <div class="team-columns">
                <div class="team-column team-blue">
                  <h4>Blue Team</h4>
                  <div id="blueTeamList" class="team-player-list"></div>
                </div>
                <div class="team-column team-red">
                  <h4>Red Team</h4>
                  <div id="redTeamList" class="team-player-list"></div>
                </div>
              </div>
              <p class="subtle team-switch-hint">Tap a player to switch teams before locking the roster.</p>
              <button id="lockRosterBtn" class="btn btn-ghost small">Lock Roster</button>
            </div>

            <div class="tip-row">
              <span class="tip-icon" tabindex="0">?</span>
              <span class="tip-text">
                Tip: When you add a player, they're automatically selected for scoring. In Team Play mode, player scores combine into team totals.
              </span>
            </div>
        ...
        <!-- Party Panel (for joining players) -->
        <div id="partyPanel" class="setup-panel">
          <div class="card">
            <h2>Join Party</h2>
            ...
            <label class="field">
              ...
              <div class="player-input-row">
                <input id="partyRoomCodeInput" type="text" maxlength="8" placeholder="e.g. 4FG9" />
                <button id="partyConnectBtn" class="btn btn-ghost small">Join</button>
              </div>
            </label>
            ...
            <label class="field">
              ...
              <div class="player-input-row">
                <input id="partyPlayerNameInput" type="text" maxlength="18" placeholder="Enter your name" />
                <button id="partyAvatarBtn" class="btn btn-ghost avatar-picker-btn" type="button">
                  <span id="partyAvatarPreview">🎮</span>
                </button>
              </div>
            </label>

            <div id="partyTeamSection" class="party-team-section hidden">
              <div class="field-label-row">
                <span class="field-label">Your Team</span>
              </div>
              <div class="team-picker">
                <button id="partyBlueTeamBtn" class="btn btn-ghost team-btn blue">Blue Team</button>
                <button id="partyRedTeamBtn" class="btn btn-ghost team-btn red">Red Team</button>
              </div>
            </div>

            <button id="partyReadyBtn" class="btn btn-cta">Ready!</button>
          </div>
          ...
          <div id="waitingRoom" class="waiting-room hidden">
            <div class="waiting-content">
              ...
              <p class="waiting-text">Waiting on host...</p>
              <p class="waiting-hint subtle">The host is picking a list. Hang tight!</p>
            </div>
          </div>
        </div>
      </section>
...
Someone yells at the ref"></textarea>
          ...
          <div class="tip-row">
            <span class="tip-icon" tabindex="0">?</span>
            <span class="tip-text">
              Gesture tip: tap an event when it happens on screen. We’ll handle the points and vetoes.
            </span>
          </div>
      ...
      <section id="screen-game" class="screen">
        <div class="screen-header">
          ...
          <button id="pauseGameBtn" class="btn btn-ghost small">Pause</button>
        </div>

        <!-- Pause Banner -->
        <div id="pauseBanner" class="pause-banner hidden">
          <span class="pause-icon">⏸</span>
          <span>Break time! Scoring is paused.</span>
        </div>

        <div class="card card-compact">
          <div class="players-bar">
            <div class="players-bar-top">
              <span class="field-label">Who’s tapping?</span>
      ...
      <!-- Screen: Open Parties -->
      <section id="screen-parties" class="screen">
        <div class="screen-header">
          <button class="btn btn-ghost" id="partiesBackBtn">&larr; Back</button>
          <div class="screen-title">
            <h2>Open Parties</h2>
            <p class="subtle">Resume saved games</p>
          </div>
        </div>

        <div class="card">
          <div id="openPartiesList" class="open-parties-list">
    ...
    <nav id="bottomNav" class="bottom-nav">
      <button id="navSettingsBtn" class="bottom-nav-btn">
        <span class="nav-icon">⚙</span>
        <span class="nav-label">Settings</span>
      </button>
      <button id="navPartiesBtn" class="bottom-nav-btn">
        <span class="nav-icon">🎮</span>
        <span class="nav-label">Open Parties</span>
      </button>
    </nav>
      ...
      <div class="overlay-card tutorial-card">
        ...
        <div id="tutorialSlidesWrapper" class="tutorial-slides-wrapper">
          <div class="tutorial-slides">
            ...
            <div class="tutorial-slide">
              <h4>2. Load a list</h4>
              <p>
                Pick a preset list for live sports, TV, movies, or family chaos – or create your own events for any watch night.
              </p>
        ...
        <div class="tutorial-actions">
          ...
          <div class="tutorial-nav">
            <button id="tutorialPrevBtn" class="btn btn-ghost small">‹</button>
    ...
    <div id="settingsOverlay" class="overlay">
      <div class="overlay-card settings-card">
        <div class="settings-header">
          <h3>Settings</h3>
          <button id="settingsCloseBtn" class="btn-icon" aria-label="Close settings">✕</button>
        </div>
        <div class="settings-body">
          <div id="settingsAuthSection" class="settings-auth-section">
            <div id="settingsUserInfo" class="settings-user-info hidden">
              <img id="settingsUserAvatar" class="settings-user-avatar" alt="User avatar" />
              <span id="settingsUserName" class="settings-user-name"></span>
            </div>
            <button id="settingsSignInBtn" class="btn btn-primary full-width hidden">Sign in with Google</button>
            <button id="settingsSignOutBtn" class="btn btn-ghost full-width hidden">Sign out</button>
          </div>
          ...
          <button id="chooseAvatarBtn" class="btn btn-ghost full-width">
            <span id="selectedAvatarPreview">🎮</span> Choose your Avatar
          </button>
    ...
    <div id="avatarModal" class="modal-overlay">
      <div class="modal-panel avatar-modal">
        <div class="modal-header">
          ...
          <button id="avatarModalCloseBtn" class="btn-icon" aria-label="Close">✕</button>
        </div>
        <div class="avatar-grid-container">
          <div id="avatarGrid" class="avatar-grid"></div>
        </div>
        <div class="modal-footer">
          <button id="avatarSaveBtn" class="btn btn-primary full-width">Save</button>
        </div>
      </div>
    </div>
  </div>
